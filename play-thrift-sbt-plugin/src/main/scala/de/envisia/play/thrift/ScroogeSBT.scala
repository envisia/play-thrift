...
import de.envisia.play.thrift.backend.{ ServiceOption, WithFinagle }
import sbt.Keys._
import sbt._
...
object ScroogeSBT extends AutoPlugin {
  ...
  private[this] def compile(
    ...
    try {
      ...
      thriftIncludes.map {
        compiler.includePaths += _.getPath
      }
  ...
  val genThriftSettings: Seq[Setting[_]] = Seq(
    ...
    scroogeThriftSourceFolder <<= (sourceDirectory) {
      _ / "thrift"
    },
    scroogeThriftExternalSourceFolder <<= (target) {
      _ / "thrift_external"
    },
    scroogeThriftOutputFolder <<= (sourceManaged) {
      identity
    },
    scroogeThriftIncludeFolders <<= (scroogeThriftSourceFolder) {
      Seq(_)
    },
